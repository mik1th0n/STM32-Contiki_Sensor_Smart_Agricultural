...
{
    static bool isCommunicationPacketReceiveEnd = true;		/* 接收完成标志 */
    static PacketBlock* packetBlock = NULL;		/* 包块指针定义 */
    uint16_t count;
    static bool isHeadAllEqual = false;		/* 包头相等检验标志 */
    
    if(!packetBlock)
    {
        packetBlock = (PacketBlock*)malloc(sizeof(PacketBlock));		/* 为包块开辟空间 */
        if(!packetBlock)return;		/* 开辟失败，返回 */
